...
class KeyOutput {
  ...
  tap(key) {
    try {
      ...
      console.error(`KeyOutput Error tapping ${key}:`, error2);
    }
  }
}
const keyOutput = new KeyOutput();
const _dirname = path.dirname(fileURLToPath(import.meta.url));
class VirtualController {
  constructor() {
    __publicField(this, "isConnected", false);
    __publicField(this, "pythonProcess", null);
    __publicField(this, "intentionalUnplug", false);
    __publicField(this, "buttonMap", {
  ...
  sendCmd(cmd) {
    var _a;
    if (!this.isConnected || !((_a = this.pythonProcess) == null ? void 0 : _a.stdin)) return;
    try {
  ...
  async tap(buttonName, durationMs = 60) {
    if (!this.pythonProcess) this.plugin();
    const ready = await this.waitUntilReady(3e3);
    if (!ready) {
  ...
  setAxes(steering, gas, brake, steerJoystick = "left") {
...
class Translator {
  constructor() {
    __publicField(this, "state", {
      currentVirtualGear: 0,
      // 0 = N, 1-6, -1 = R
      lastStableSlot: 0,
      lastActionTime: 0
    });
    __publicField(this, "pendingSlot", null);
    __publicField(this, "debounceTimeout", null);
    __publicField(this, "latestClutchValue", 0);
    ...
    __publicField(this, "onUpdateCallback", null);
  }
  update(slot, axes, clutchIdx, steerIdx, throttleIdx, brakeIdx) {
    let clutchValue = 0;
    let steer = 0;
    let gas = 1;
    let brake = 1;
  ...
  processShift(targetSlot) {
    ...
    const diff2 = targetSlot - current;
    if (diff2 === 0) return;
    this.executeShift(diff2, targetSlot);
  }
  async executeShift(diff2, targetGear) {
    let count = Math.abs(diff2);
    console.log(`Translating: ${this.state.currentVirtualGear} -> ${targetGear} (${diff2 > 0 ? "Up" : "Down"} x${count})`);
    this.state.lastActionTime = Date.now();
    for (let i = 0; i < count; i++) {
      if (this.settings.mode === "vigem") {
        const btn = diff2 > 0 ? this.settings.shiftUpButton : this.settings.shiftDownButton;
        virtualController.tap(btn, this.settings.pressDelayMs);
      } else {
        const key = diff2 > 0 ? this.settings.shiftUpKey : this.settings.shiftDownKey;
        keyOutput.tap(key);
      }
      if (i < count - 1) {
...
class InputHandler {
  constructor() {
    ...
    // Gear -> Button Index
    __publicField(this, "gearMapping", {
      1: 12,
      2: 13,
      3: 14,
      4: 15,
    ...
    __publicField(this, "clutchAxisIndex", 1);
    __publicField(this, "steeringAxisIndex", 0);
    __publicField(this, "throttleAxisIndex", 2);
    __publicField(this, "brakeAxisIndex", 3);
    __publicField(this, "currentSlot", 0);
    __publicField(this, "currentClutch", 0);
  }
  setAxesIndices(clutch, steering, throttle, brake) {
  ...
  updateMapping(gear, buttonIndex) {
...
let win;
let _lastAxes = [];
ipcMain$1.on("input:state", (_event, state) => {
  if (state.axes) _lastAxes = state.axes;
  inputHandler.handleState(state);
});
ipcMain$1.handle("debug:status", () => {
  return {
...
ipcMain$1.handle("calibration:get", () => {
  return {
...
ipcMain$1.on("translator:toggle", (_event, enabled) => {
  translator.setEnable(enabled);
  console.log("Translator enabled:", enabled);
});
ipcMain$1.on("calibration:save", (_event, data) => {
...
ipcMain$1.on("translator:updateSettings", (_event, settings) => {
...
ipcMain$1.on("output:test", (_event, key) => {
  keyOutput.tap(key);
});
ipcMain$1.on("output:testVigem", (_event, button) => {
  virtualController.plugin();
  virtualController.tap(button, 800);
});
function createWindow() {
  ...
  translator.onUpdate((state) => {
    win == null ? void 0 : win.webContents.send("translator:update", state);
  });
